...
@inlinable public func makeCString(from str: String, count: inout Int) -> UnsafeMutablePointer<Int8> {
...
@inlinable public func generatePySwiftMethod(name: String, _func: PyCFunction!, docs: String!) -> UnsafeMutablePointer<PyMethodDef> {
    var count = 0
    let method_name_ptr: UnsafeMutablePointer<CChar> = makeCString(from: "spam", count: &count)
...
public class PySwiftMethod: Hashable {
    ...
    public init(_module: PythonPointer ,func_name: String, _func: @escaping PyCFunction) throws {
        var ncount = 0
        let _name = makeCString(from: func_name, count: &ncount)
        name = _name
        name_count = ncount
        swiftFunc = _func
